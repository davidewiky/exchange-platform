package ccf.org.platform.model.common;

import ccf.org.platform.model.util.ExcludeFromToString;
...
/**
 * This is the basic abstract entity implementation. All JPA entities should extends this basic entity. Basically this
 * class force to define a unique ID and the methods equals and hashCode based on it.
 */
public abstract class AbstractEntity implements Cloneable,Serializable {
	...
	protected String buildToString(final Class<? extends Object> clazz) {
		...
		for (final Field field : fields) {
			...
			if (!AbstractEntity.class.isAssignableFrom(field.getType()) && !Collection.class.isAssignableFrom(field
					.getType())) {

				boolean log = true;
				final Annotation[] annotations = field.getAnnotations();
				for (final Annotation annotation : annotations) {
					if (annotation.annotationType() == ExcludeFromToString.class || annotation
							.annotationType() == ManyToOne.class || annotation.annotationType() == MapsId.class) {
						log = false;
					}
				}
