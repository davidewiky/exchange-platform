package ccf.org.platform.logic.jpa;

import ccf.org.platform.logic.search.QueryAttributes;
import ccf.org.platform.logic.search.SortMode;
...
/**
 * This class contains utility JPA methods that simplify recurrent operations of queries and objects. It is an generic
 * JPA helper.
 */
...
public class CriteriaHelper {
	...
	/**
	 * This method verify if a specific attribute of an object (example) can be applied to a query. It simply checks if
	 * attribute of the example passed as argument is not null and has a meaningful value. The current implementation
	 * only provide support for String and Long. It can be easily extended.
	 *
	 * @param <T> The type of the example
	 * @param example The object to be verified.
	 * @param attribute The attribute to be checked.
	 * @return true if the attribute is not null and has a meaningful value (if string type then it must be with a
	 *         length different from 0 / if long it must be different from -1)
	 */
	...
	/**
	 * This method takes a container of a CriteriaQuery already created and apply all the sort attributes specified by
	 * the attribute parameter.
	 *
	 * @param <T> The type of ROOT specified in the criteria query.
	 * @param cb The reference to the criteria builder already created in the calling method.
	 * @param container The container of the criteria query.
	 * @param attributes The object containing the specification of the query attributes to be applied (sort/pager)
	 */
	...
	/**
	 * This method simply creates a SingleRootCriteriaQuery starting from the class that will be the root of the
	 * criteria with no fetches.
	 *
	...
	/**
	 * This method simply creates a SingleRootCriteriaQuery starting from the class that will be the root of the
	 * criteria and the list of fetches defined by a list of attributes of the class.
	 *
	 * @param <T> The class type of the root.
	...
	/**
	 * In some implementation of the JPA specification the ilike predicate is not available. Due to the fact that in
	 * Hibernate the ilike predicate is really useful when creating search engine, this method simply build such a
	 * predicate starting from the attribute and the filter to be applied.
	 *
	...
	/**
	 * This method simply build such a predicate starting from the attribute and the filter to be applied.
	 *
	 * @param <T> The type T of the root on which the predicate will be created.
	 * @param root The root on which the predicate will be applied.
	 * @param cb The criteria builder in order to access the basic predicates.
	...
	/**
	 * In some implementation of the JPA specification the Query-By-Example utility is not available. This method
	 * created a list of predicate on the basis of the provided example. It checks if the String attributes can be
	 * applied into the query. If the attribute in the example is not null and with a valid value (length>0) then a
	 * predicate will be created using like clause and added to the list.
	 *
	 ...
	 * created a list of predicate on the basis of the provided example. It checks if the String attributes can be
	 * applied into the query. If the attribute in the example is not null and with a valid value (length>0) then a
	 * predicate will be created using equal clause and added to the list.
	 *
	 * @param <T> The type of the root.
	 * @param root The root on which the predicate will be applied.
	 * @param cb The criteria builder in order to access the basic predicates.
	...
	/**
	 * This method checks if the EMappable attributes can be applied into the query. If the attribute in the example is
	 * not null, then a predicate will be created using equal clause and added to the list.
	 *
	 * @param root The root on which the predicate will be applied.
	 * @param cb The criteria builder in order to access the basic predicates.
	 * @param example The provided example in order to specify the values of the predicates.
	...
	/**
	 * This method simply extract the value of a specified attributes form an existing object.
	 *
	 * @param instance The object from which the value of the attribute must be extracted.
	...
	private Object getValue(final Object instance, final SingularAttribute<?, ?> attribute) {
		final String methodName = "get" + attribute.getName().substring(0, 1).toUpperCase() + attribute.getName()
				.substring(1);
		Method method;
		try {
	...
	/**
	 * This method load the metamodel of an example class of type T. The * metamodel is the generated class with the
	 * "_". The fields list is made of Attribute objects.
	 *
	 * @param <T> The type of the class to be checked.
	 * @param exampleClass The class that will be used for searching the corresponding metamodel class.
